...
module.exports = (db, updateNotification) => {
  router.get("notifications/", (request, response) => {
    db.query(
      ...
      JOIN amily_members.family_id = families.id
      WHERE users.id = $1::integer AND family_members.family_id = families.id
    ...
    `,
      ...
      .then(() => {
        setTimeout(() => {
          ...
          updateNotification(Number(request.params.id), request.body.notification);
        }, 1000);
      })
